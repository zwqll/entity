...
class Lable(object):
    def __init__(self):
        self.entities = ['symp', 'disease', 'drug']
        self.labels = ['SYM', 'DIS', 'DRU']
        self.path = '../data/'
        if not os.path.exists(path):
            os.makedirs(path)
    def run(self):
        self.originalcsv2postxt()
        self.match_entities()
        self.nerlabeling_seg_again()
        self.SplitDataset()
    
    def originalcsv2postxt(self, textdata):
        """
        对原始语料进行分词，并用jieba获取每个词的词性，每句话用空行分隔
        """
        seg_list = segment(self, textdata)
    ...
    def label(self, label, templines, i, word, pos):
    """
    工具方法，打标签
    """
    if label == 'O':
    ...
    def nerlabel_for_sentence(self, len_tag_list, label_type):
        """
    ...
    def nerlabeling_seg_again(self):
        """
        二次分词函数入口，写入文件中
        """
        disease_dict = []
