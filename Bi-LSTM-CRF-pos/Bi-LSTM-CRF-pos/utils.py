...
def pad_sentence_pos_tags(sentences):
    """
    Pad the pos tags of the sentence in the batch_sentence.
    Input:
        - list of lists of ints (list of sentence, a sentence being a list of pos tags indexes)
    Output:
        - padded list of lists of ints
        - padded list of lists of ints (where chars are reversed)
        - list of ints corresponding to the index of the last character of each word
    ...
    for pos_tags in sentences:
        padding = [0] * (max_length - len(pos_tags))
        pos_tags_for.append(pos_tags + padding)
        pos_tags_pos.append(len(pos_tags) - 1)
    return pos_tags_for, pos_tags_pos
...
def create_input_batch(sentences, parameters, n_tags=0, add_label=False, singletons=None):
    ...
    # pos
    pos_tags_for, pos_tags_pos = pad_sentence_pos_tags(pos_tags_batch)
    input.append(pos_tags_for)
